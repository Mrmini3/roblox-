--!strict
-- FieldOps (stub): door-knock runner
...
function Module.Start(context: {[string]: any})
	...
	active = true
	Remotes.RE_MiniGame_Start():FireAllClients("FieldOps", {stub=true})
	task.delay(math.min(context.MatchRoundSeconds, 10), function()
		Remotes.RE_MiniGame_End():FireAllClients({name="FieldOps", RoundResults={}})
		active = false
		ended:Fire({name="FieldOps", RoundResults={}})
	end)
